...
if __name__ == "__main__":
	...
	parser.add_argument("--language", type=str, default='english_1')
	...
	if args["feature_level"] == "word":
		...
		layers.CNN_models.train_word_cnn(use_glove, use_ctxt, index, batch_size, num_epochs, filter_sizes, num_filters, train_embedding, lr, time_index,args['language'],gl_txt)
		...
		time_index = int(time.time())
		# for index in range(args['num_splits']):
		layers.CNN_models.train_char_cnn(index, batch_size, num_epochs, filter_sizes, num_filters, train_embedding, lr, time_index,args['language'],gl_txt)
		...
		time_index = int(time.time())
		# for index in range(args['num_splits']):
		layers.CNN_models.train_hybrid_cnn(use_glove, index, batch_size, num_epochs, filter_sizes_word, num_filters_word, filter_sizes_char, num_filters_char, train_embedding, lr, time_index,args['language'],gl_txt)
