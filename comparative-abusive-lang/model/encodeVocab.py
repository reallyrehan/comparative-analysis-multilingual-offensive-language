...
def genWordEmbeddings(ues_glove, path):
	...
	if ues_glove:
		...
		f = open(f"../../embeddings/{embedding_file}",encoding= "ISO-8859-1")
...
def processWordVocab(target_path, use_glove, k,language):
	print("Processing word vocab....")
	# data_path = os.path.dirname(os.path.abspath(__file__)) + "/../data"

	
	with open(f"../data/cnn/{language}_cnn.pkl", "rb") as f:
		_data = pickle.load(f)
...
def processCharVocab(target_path, k,language):
...
if __name__ == "__main__":
	...
	parser.add_argument("--language", type=str, default='english_0')
	...
	processWordVocab(target_path, args["use_glove"], args["num_splits"],args['language'])
	processCharVocab(target_path, args["num_splits"],args['language'])
