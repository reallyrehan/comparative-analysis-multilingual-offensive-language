...
def returnLabels():
	global language
	with open(f"../../processed_data/{language}_processed.pkl", "rb") as f:
		_data = pickle.load(f)
		return list(_data['label_dict'].keys())
...
def train_word_cnn(use_glove, use_ctxt, kfold_index, batch_size, num_epochs, filter_sizes, num_filters, train_embedding, lr, time_index,lang,gl_txt):
	global language
	language = lang
	...
	# print("LOOKHERE")
	# print(log_path + "/weights.{epoch:02d}.hdf5")
	...
	if use_ctxt:
		...
		print(text_data["train"].shape)
		print(ctxt_data['train'].shape)
		print(label_data['train'].shape)
		print(labels)
	...
	model.model.save(f'../data/target/models/{lang}_model{gl_txt}word')
	# print("HERE2")
	print("Training Finished")

def train_char_cnn(kfold_index, batch_size, num_epochs, filter_sizes, num_filters, train_embedding, lr, time_index,lang,gl_txt):
	global language
	language = lang
	...
	model.model.save(f'../data/target/models/{lang}_model{gl_txt}char')

	print("Training Finished")

def train_hybrid_cnn(use_glove, kfold_index, batch_size, num_epochs, filter_sizes_word, num_filters_word, filter_sizes_char, num_filters_char, train_embedding, lr, time_index,lang,gl_txt):
	global language
	language = lang
	...
	model.model.save(f'../data/target/models/{lang}_model{gl_txt}hybrid')
